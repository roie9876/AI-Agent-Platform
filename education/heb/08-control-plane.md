...
### תרחיש מציאותי: קליטת לקוח חדש (Onboarding)
תארו לעצמכם שהחברה שלכם כרגע חתמה עם חברת ענק: "אקמה קורפ". הם צריכים גישה למערכת סוכני ה-AI שלכם.
במקום להרים שרתים חדשים או לכתוב קוד מחדש, ה-DevOps פשוט מתחבר למסך של ה-**Control Plane** ולוחץ על כפתור יצירת פרויקט: `TenantID='acme-corp'`.
באותה שנייה, ה-Control Plane מנפיק להם מפתחות API, מגדיר להם תקציב גג של $500 בחודש, ופותח להם הרשאה רק להריץ סוכנים מקטלוג "שירות הלקוחות". ה"מסלול ההמראה" הפיזי (Runtime) נשאר ללא שינוי, אבל מגדל הפיקוח אישר למטוס חדש להמריא תחת חוקים ברורים.
...
### דוגמה: קובץ הגדרות Agent

הנה איך נראית הגדרת Agent אמיתית ב-YAML:

```yaml
# agent-config.yaml - הגדרת Agent לדוגמה
...
spec:
  ...
  policies:
    - no-pii-output        # הפנייה למדיניות DLP
    - max-cost-daily-50     # מגבלת תקציב

  limits:
    max_steps: 10           # מקסימום איטרציות ReAct
    timeout_seconds: 120    # זמן מקסימלי לבקשה
...
spec:
  model:
    deployment: gpt-4o-prod  # deployment שונה בפרודקשן
    temperature: 0.1          # יותר דטרמיניסטי בפרודקשן
  limits:
    max_steps: 8              # יותר מחמיר בפרודקשן
    timeout_seconds: 60
```
...
spec:
  model:
    deployment: gpt-4o-mini-dev  # מודל זול יותר לפיתוח
    temperature: 0.7              # יותר יצירתי לבדיקות
  limits:
    max_steps: 20                 # יותר מקל לפיתוח
    timeout_seconds: 300
```
