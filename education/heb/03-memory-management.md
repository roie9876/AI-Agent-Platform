...
### אנלוגיית "הלוח המחיק" (The Whiteboard Analogy)
תחשבו על ה-Context Window של ה-LLM כמו על לוח מחיק פיזי בחדר ישיבות.
- אתם יכולים לכתוב על הלוח את כל היסטוריית השיחה כדי שהמודל "יזכור" על מה דיברתם.
- אבל ללוח יש גודל סופי (למשל 128K טוקנים). 
- כשהלוח מתמלא, אי אפשר להוסיף יותר מידע. אתם נאלצים למחוק את המשפטים הישנים ביותר מתחילת הפגישה (Sliding Window), או לקרוא את כל הלוח, לכתוב סיכום של משפט אחד בפינה, ולמחוק את השאר (Summary Memory).
- **RAG** הוא כמו ארון תיוק אינסופי שעומד לצד הלוח. כשמישהו שואל שאלה, אתם מוצאים בארון בדיוק את שני הדפים הרלוונטיים, ומדביקים אותם על הלוח רק לצורך התשובה הזו.
...
## נוף התעשייה: ניהול זיכרון ו-RAG בייצור

כיצד חברות מנהלות זיכרון בסביבת Production כיום?

| כלי מחקר / מסד נתונים | שימוש מרכזי | למה לבחור בו? |
|-----------------------|-------------|---------------|
| **Pinecone / Milvus / Qdrant** | מסדי נתונים וקטוריים ייעודיים ל-RAG | סקיילינג למאות מיליוני מסמכים, חיפוש אלפיות שנייה (High Performance) |
| **pgvector (PostgreSQL)** | הוספת תמיכה וקטורית למסד נתונים רלציוני קיים | מונע את הצורך בניהול מסד נתונים חדש לחלוטין. הכל נשאר ב-SQL |
| **Zep / Mem0** | שירותי זיכרון לסוכני AI (Memory-as-a-Service) | מנהלים אוטומטית Extraction של עובדות מהשיחה לזיכרון ארוך-טווח בלי קוד |
| **Redis** | ניהול היסטוריית שיחות (Short-Term Memory) | מהיר מאוד לשליפה של רשימת ה-Messages האחרונה של המשתמש (Session ID) |
