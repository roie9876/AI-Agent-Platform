...
### תרחיש מציאותי: קריסת הדפדפן
תארו לעצמכם שמשתמש נעזר בסוכן (Agent) כדי לכתוב תוכנה מורכבת. הסוכן כבר חקר הרשאות, בנה מבנה בסיס נתונים וסיים 3 מתוך 5 קבצים. פתאום - הסוללה של המשתמש נגמרת, או שהדפדפן קורס.
- **ללא ניהול State / Threads:** התהליך הולך לאיבוד. כשהמשתמש רענן את העמוד, הסוכן אומר "שלום, איך אפשר לעזור היום?". חצי שעת עבודה ירדה לטימיון.
- **עם ניהול Checkpointing ו-State:** המשתמש נכנס מחדש, לוחץ על `Thread 2`, והסוכן ממשיך מאותה נקודה בדיוק: `Step 3 (Generating tests)`. התהליך מוקפא ומופשר ללא בעיות.
...
graph TB
    ...
    User --> T1["🧵 Thread 1: 'נתח מכירות'<br/>📅 10:00"]
    User --> T2["🧵 Thread 2: 'כתוב קוד'<br/>📅 14:00"]
    User --> T3["🧵 Thread 3: 'עזרה בחוזה'<br/>📅 16:30"]
...
graph TB
    ...
    TM --> Create["✨ Create<br/>יצירת thread חדש"]
    TM --> Load["📂 Load<br/>טעינת thread קיים"]
    TM --> Append["➕ Append<br/>הוספת הודעות"]
    TM --> Fork["🔀 Fork<br/>פיצול thread"]
    TM --> Archive["📦 Archive<br/>ארכוב thread ישן"]
    TM --> Delete["🗑️ Delete<br/>מחיקה"]
```
...
```mermaid
graph LR
    Simple["🤖 Agent פשוט<br/>שאלה → תשובה<br/>(Stateless)"]
    Complex["🤖 Agent מורכב<br/>Workflow עם שלבים<br/>(Stateful)"]
```
...
graph TB
    ...
    Decision -->|"❌ שליחת מייל לכל החברה"| HITL["⏸️ HITL<br/>חכה לאישור"]
...
## נוף התעשייה: ניהול State ו-Threads כיום

| Framework | פתרון ניהול ה-State | איך זה עובד למפתחים? |
|-----------|---------------------|----------------------|
| **LangGraph** | מבוסס State Graph | מנהל מילון (Dictionary) גלובלי שמגיע לכל Node (צומת), וכל צומת שומר לו שינויים אל צ'קפוינטר מבוסס Postgres או SQLite |
| **AWS Step Functions** | מנוע State מנוהל (SaaS) | משתמש ב-JSON כדי להעביר State בין פונקציות למבנה זרימה גרידי. מאוד רובסטי אך יקר וללא אינטגרציה טבעית במיוחד ל-LLMs |
| **Temporal (Uber OSS)** | פלטפורמת Durable Executions | קוד רגיל שמשתמש ביכולות Time Travel ו-Retry אינסופי מאחורי הקלעים כדי לוודא ששום State לא יילך לאיבוד גם בקריסת שרת מרכזי |
| **Azure Durable Functions** | פונקציות מתמשכות (Serverless) | קוד .NET או פייתון רגיל ששומר את כל ההיסטוריה לתוך Storage Queue. מעולה לסביבות מיקרוסופט |
